...
def instrumentTransmission(wavelength, constant = None, path = None):

    """
    Calculate the instruemnt's transmission either as a constant value or from a given file.
    ...
    constant : float, optional
        Constant value for the instrumental transmission. If provided, the function will return 
        an array with this constant value at each given wavelength. Default is None.

    path : str, optional
        Path to a CSV file containing the instrument's transmission as a function of wavelength. 
    ...
    -------
    array
        Array of the instrument's transmission values at the given wavelengths.
    ...
    >> instrumentTransmission(wavelengths, constant=0.9)
    array([0.9, 0.9, 0.9])
    """
    ...
    wave = df.iloc[0]
    instrument_transmission = df.iloc[1]

    # Create a cubic spline interpolation
    cs = CubicSpline(wave, instrument_transmission)

    # Using the spline, evaluate the flux at the preferred wavelengths
    instrument_transmission = cs(wavelength)

    return instrument_transmission