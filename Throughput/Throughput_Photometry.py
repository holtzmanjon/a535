...
class FullWidth:
    ...
    def seeing(self):
        ...
        median_fwhm = FullWidth.GetFWHM(self.data)
...
class Photometry:
    ...
    # Initialize the class
    def __init__(self, data, header, standard_M, spec_table, fwhm):

        # Define the atrributes
        self.data = data
        self.header = header
        self.standard_M = standard_M
    ...
    def ap_phot(self, source_positions):
        ...
        # Check if using a standard
        if self.standard_M != None:
...
class Detector:
    ...
    # Initialize the class
    def __init__(self, data, header, standard_spec, filter, filter_response, standard_spec_table, fwhm):
    ...
    def throughput(self):
        ...
        photon_flux = (filtered_flux*wavelength)/(h*c)

        # Area of the telescope
        Area = (60.96/2)**2*np.pi  # cm^2

        # Convert to photons per second
        true_count_flux = simps(photon_flux, wavelength) * Area
        ...
        phot = Photometry.ap_phot(self, source_positions)

        # Extract the photometry
        observed_count_flux = phot[self.filter + ' counts_sec']
